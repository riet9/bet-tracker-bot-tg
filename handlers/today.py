...
async def prompt_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'prompt_morning':
        text = (
            ...
            "Основной упор — на логичные value-ставки. Ставки #safe, #sharp и #wildcard предлагай только если они действительно оправданы и дают явную ценность. "
            "Не ищи их ради количества или баланса формата.\n\n"
            "Не спеши. Используй столько времени, сколько нужно. Перепроверяй данные, избегай ошибок. Не концентрируйся только на популярных рынках — ищи реальную ценность даже в нестандартных. "
            "Цель — найти лучшие и самые обоснованные варианты, а не просто заполнить список.\n\n"
            ...
            "Не имитируй анализ. Не используй поверхностные объяснения или шаблонные доводы вроде “эта команда недавно выиграла — значит победит снова”. Каждый прогноз должен быть логически непротиворечивым, аналитически обоснованным и статистически целесообразным.\n"
            "Сравни команды по нескольким ключевым показателям, не выноси суждения на основании одного факта. Проверяй гипотезу перед выводом: “действительно ли это событие имеет value?” Если нет — не предлагай.\n\n"
            "Банк:\n"
            ...
            "Относись к задаче серьёзно. Это часть системной стратегии, а не просто развлечение. Работай точно, как будто от твоего анализа зависит результат.\n"
        )
    else:
        text = (
            "Я немного ориентируюсь в футболе, хоккее и CS2, но доверяю системе полностью.\n\n"
